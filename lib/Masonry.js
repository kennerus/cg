        ...
        for ( var i = 0; i < this.options.cards.length; i++ ) {
            ...
            if (i >= this.options.cols) { // Проходим по всем элементам, начиная с того, который не попал в первую строку (метод FirstRow())
                ...
                if (this.options.cards[i].classList.contains('card-2')) {
                    ...
                    if (this.smaller.offsetLeft > this.smallerSibling.offsetLeft) {
                        this.options.cards[i].style.left = this.smallerSibling.offsetLeft + 'px';
                    }
                    else {
                    ...
                    if (this.smaller.classList.contains('card-2') && flag) {
                        flag = false;
                        this.options.cards[i].style.left = (this.smaller.offsetWidth + this.defaultParams.columnGapRight) / 2 + this.smaller.offsetLeft + 'px';
                    }
                    else {
                        flag = true;
        ...
        for (var i = 0; i < topEl.length; i++) {
            if (this.smaller.offsetTop + this.smaller.offsetHeight > topEl[i].offsetTop + topEl[i].offsetHeight) {
                ...
                if (this.smallerNextSibling.offsetHeight + this.smallerNextSibling.offsetTop > this.smallerPrevSibling.offsetHeight + this.smallerPrevSibling.offsetTop) {
                    ...
                    this.smallerSibling = this.smallerNextSibling;
                    this.smallerSiblingHeight = this.smallerNextSibling.offsetHeight + this.smallerNextSibling.offsetTop;
                }
            }
        }
        this.smaller.classList.remove('topEl'); // Удаляем у него класс
        return this.smaller, this.smallerSiblingHeight, this.smallerSibling; // Возвращаем его
    };
}
